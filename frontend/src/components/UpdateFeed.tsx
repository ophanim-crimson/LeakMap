...
const UpdateFeed: React.FC<UpdateFeedProps> = ({ report, onUpdate }) => {
  ...
  const handleSubmitUpdate = async () => {
    ...
    try {
      ...
      message.error(err.response?.data?.detail || t('Failed to submit update.'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Card 
      title={<div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '15px' }}><CommentOutlined style={{ color: 'var(--primary-color)' }} /> {t('Community Updates')}</div>}
    ...
    >
      ...
      <div style={{ marginBottom: '24px' }}>
        <TextArea
          ...
          placeholder={t("Share progress or changes (e.g., municipal crew arrived, leak has worsened, tap replaced)...")}
          maxLength={1000}
      ...
      <div>
        {report.updates.length === 0 ? (
          <Empty 
            description={t("No updates posted yet. Be the first to share details!")} 
