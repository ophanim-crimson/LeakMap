...
const VerificationPanel: React.FC<VerificationPanelProps> = ({ report, onUpdate }) => {
  ...
  return (
    <Card 
      title={<div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '15px' }}><InfoCircleOutlined style={{ color: 'var(--primary-color)' }} /> {t('Community Verification')}</div>}
      bodyStyle={{ padding: '20px' }}
      style={{ border: '1px solid var(--border-color)' }}
    >
      <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '16px' }}>
        {t('Help verify this report. If you are near this location, please confirm if this issue is active, resolved, or duplicate.')}
      </p>
      ...
      <div style={{
        ...
        <div style={{ backgroundColor: '#E8F5E9', padding: '10px', borderRadius: 'var(--radius-sm)' }}>
          <span style={{ display: 'block', fontSize: '11px', color: 'var(--text-secondary)', fontWeight: 500 }}>{t('CONFIRMED')}</span>
          <span style={{ fontSize: '20px', fontWeight: 700, color: 'var(--success-color)' }}>
            {report.verification_counts?.confirmed || 0}
          </span>
        </div>
        <div style={{ backgroundColor: '#FFF3E0', padding: '10px', borderRadius: 'var(--radius-sm)' }}>
          <span style={{ display: 'block', fontSize: '11px', color: 'var(--text-secondary)', fontWeight: 500 }}>{t('DUPLICATE')}</span>
          <span style={{ fontSize: '20px', fontWeight: 700, color: 'var(--warning-color)' }}>
            {report.verification_counts?.duplicate || 0}
          </span>
        </div>
        <div style={{ backgroundColor: '#E3F2FD', padding: '10px', borderRadius: 'var(--radius-sm)' }}>
          <span style={{ display: 'block', fontSize: '11px', color: 'var(--text-secondary)', fontWeight: 500 }}>{t('RESOLVED')}</span>
          <span style={{ fontSize: '20px', fontWeight: 700, color: 'var(--primary-color)' }}>
            {report.verification_counts?.resolved || 0}
          </span>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        >
          {report.status === 'Resolved' ? t('Verify Still Resolved') : t('Mark as Fixed / Resolved')}
        </Button>
      </div>
    </Card>
  );
