...
const LeafletMap: React.FC<LeafletMapProps> = ({
  ...
  return (
    ...
    <div style={{
      ...
      <div style={{ fontWeight: 700, marginBottom: '4px', fontSize: '11px', textTransform: 'uppercase', color: '#64748B' }}>Legend</div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
        <span style={{ width: '10px', height: '10px', borderRadius: '50%', backgroundColor: '#F44336', display: 'inline-block' }}></span> Active
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
        <span style={{ width: '10px', height: '10px', borderRadius: '50%', backgroundColor: '#4CAF50', display: 'inline-block' }}></span> Fixed
      </div>
    </div>
    <MapContainer 
      center={mapCenter} 
            ...
            <Popup minWidth={200}>
              <div className="map-popup-card">
                ...
                {report.photos && report.photos.length > 0 && (
                  <img
                    src={report.photos[0].image_url.startsWith('http') 
                      ? report.photos[0].image_url 
                      : `${import.meta.env.VITE_API_URL || ''}${report.photos[0].image_url}`
                    }
