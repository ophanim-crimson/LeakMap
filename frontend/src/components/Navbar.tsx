    ...
    <header style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', maxWidth: '1200px', margin: '0 auto', flexWrap: 'wrap', gap: '10px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          {/* Language Toggle option (Before Admin Dashboard) */}
          <button
          ...
          >
            <Globe size={14} />
            {i18n.language === 'en' ? 'ML (മലയാളം)' : 'EN (English)'}
          </button>
          ...
          {user ? (
            <>
              ...
              {user.role === 'admin' ? (
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '5px 12px', borderRadius: '8px', backgroundColor: '#EFF6FF', color: '#1565C0', fontWeight: 700, fontSize: '12px', border: '1px solid #BFDBFE' }}>
                  <Shield size={13} />
                  {t('Admin')}
                </span>
              ) : (
            ...
            // Removed redundant Login/Register buttons from navbar
            null
          )}
        </div>
      </div>
