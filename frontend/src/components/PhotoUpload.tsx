...
const PhotoUpload: React.FC<PhotoUploadProps> = ({ photos, onChange }) => {
  ...
  const processAndUploadFile = async (file: File) => {
    // 1. Format check
    const filename = file.name.lowerCase ? file.name.toLowerCase() : file.name;
    ...
    // 3. Count limit check (5 photos max)
    if (photos.length >= 5) {
      setErrorMsg('Maximum of 5 photos allowed per report.');
      return;
    }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <span>Photos: <strong>{photos.length} / 5</strong></span>
