...
import { Row, Col, Card, Tag, Button, Spin, Typography, Result, Space, Input } from 'antd';
import { 
  ArrowLeftOutlined, 
  CalendarOutlined, 
  EnvironmentOutlined,
...
import { fetchReportById, addComment, Report, getImageUrl } from '../api';
  ...
  if (error || !report) {
    return (
      <div style={{ maxWidth: '600px', margin: '40px auto', padding: '0 16px' }}>
        <Result
          status="warning"
          title={t('Error Loading Report')}
          subTitle={error ? t(error) : t('The report you are looking for does not exist or has been removed.')}
          extra={
      ...
      <div style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '16px' }}>
        ...
        <div>
          <span style={{ fontSize: '13px', color: 'var(--text-muted)', fontWeight: 600 }}>
            {t('REPORT DETAIL')} • {report.report_code}
          </span>
      ...
      <Row gutter={[24, 24]}>
        ...
        <Col xs={24} md={14}>
          <Space direction="vertical" size="large" style={{ width: '100%' }}>
            ...
            <Card bodyStyle={{ padding: '24px' }} style={{ border: '1px solid var(--border-color)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                ...
                {report.status === 'Resolved' ? (
                  <Tag color="success" style={{ borderRadius: '12px', padding: '2px 10px', fontSize: '13px' }}>{t('Resolved')}</Tag>
                ) : (
                  <Tag color="error" style={{ borderRadius: '12px', padding: '2px 10px', fontSize: '13px' }}>{t('Active')}</Tag>
                )}
              </div>
                ...
                <div style={{ 
                  ...
                  <div>
                    ...
                    <div style={{ 
                      ...
                      {t(report.ai_urgency)} {t('Priority')}
                    </div>
                  </div>
                </div>
              )}
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <Paragraph style={{ fontSize: '15px', color: 'var(--text-primary)', lineHeight: '1.6' }}>
                  {report.description ? (
                    ...
                    <span style={{ color: 'var(--text-muted)', fontStyle: 'italic' }}>{t('No description provided')}</span>
                  )}
                </Paragraph>
              </div>
            ...
            {report.photos && report.photos.length > 0 && (
              <Card 
                title={<div style={{ fontSize: '15px' }}><CameraOutlined /> {t('Attached Photos')} ({report.photos.length})</div>}
            ...
            <Card 
              title={<div style={{ fontSize: '15px' }}><EnvironmentOutlined /> {t('Location Map')}</div>}
              extra={
                <Button 
                  type="text" 
                  icon={isMapFullscreen ? <FullscreenExitOutlined /> : <FullscreenOutlined />} 
                  onClick={() => setIsMapFullscreen(!isMapFullscreen)} 
                  title={isMapFullscreen ? t("Minimize Map") : t("Enlarge Map")}
                />
              }
              bodyStyle={{ 
                padding: 0, 
            ...
            >
              <div style={{
                ...
                borderRadius: isMapFullscreen ? '0' : '0 0 var(--radius-md) var(--radius-md)',
                overflow: 'hidden'
              }}>
                <LeafletMap
                  reports={[report]}
        ...
        <Col xs={24} md={10}>
          <Space direction="vertical" size="large" style={{ width: '100%' }}>
            ...
            {/* Comment to Admin Form (only for regular users, hidden for admin) */}
            {user && user.role !== 'admin' && (
              <Card 
                title={<div style={{ fontSize: '15px' }}>💬 {t('Add Comment to Admin')}</div>}
              ...
              >
                <Input.TextArea
                  ...
                  placeholder={t("Describe additional details about this issue...")}
                  rows={3}
                  maxLength={1000}
                ...
                <Button 
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const updated = await addComment(report.id, { text: commentText });
                      setReport(updated);
                      setCommentText('');
                    } catch (err) {
                      console.error(err);
            ...
            {/* Comments List (visible to admin) */}
            {report.comments && report.comments.length > 0 && (
              <Card
                title={<div style={{ fontSize: '15px' }}>📋 {t('Comments') || 'Comments'} ({report.comments.length})</div>}
                bodyStyle={{ padding: '16px' }}
                style={{ border: '1px solid var(--border-color)' }}
              >
                {report.comments.map(c => (
                  <div key={c.id} style={{ padding: '10px 0', borderBottom: '1px solid var(--border-color)' }}>
                    <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px' }}>
                      {c.user?.email?.split('@')[0] || 'User'} • {new Date(c.created_at).toLocaleDateString()}
                    </div>
                    <div style={{ fontSize: '14px', color: 'var(--text-primary)' }}>{c.text}</div>
                  </div>
                ))}
              </Card>
            )}
        ...
        >
          ...
          <div style={{
            ...
            <div style={{
              ...
              <img
                src={getImageUrl(report.photos[activePhotoIndex].image_url)}
                alt="Fullscreen View"
                style={{
                  maxWidth: '100%',
                  maxHeight: '100%',
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '16px', backgroundColor: 'rgba(255,255,255,0.15)', padding: '6px 16px', borderRadius: '20px' }}>
              <Button type="link" onClick={handleZoomOut} disabled={zoomScale <= 1} style={{ color: '#FFF', padding: 0 }}>{t('Zoom Out')}</Button>
              <span style={{ color: 'rgba(255,255,255,0.3)' }}>|</span>
              <Button type="link" onClick={handleZoomIn} disabled={zoomScale >= 3} style={{ color: '#FFF', padding: 0 }}>{t('Zoom In')}</Button>
            </div>
          </div>
        </div>
      )}
