...
export const uploadPhoto = async (file: File): Promise<{ image_url: string; file_size: number }> => {
  const formData = new FormData();
  formData.append('file', file);
  const response = await api.post<{ image_url: string; file_size: number }>('/api/uploads', formData, {
    headers: {
