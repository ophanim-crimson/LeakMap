...
def classify_photo_with_ai(image_bytes: bytes) -> dict:
    ...
    if not hf_token:
        return {"is_valid": False, "label": "AI verification unavailable (no API token)"}
    try:
        ...
        for attempt in range(3):
            resp = requests.post(caption_url, headers=headers, data=image_bytes, timeout=45)
            if resp.status_code == 503:
                # Model is loading, wait and retry
                wait_time = (attempt + 1) * 10
                print(f"HF model loading (attempt {attempt+1}/3), waiting {wait_time}s...")
                time.sleep(wait_time)
                continue
            elif resp.status_code == 429:
                wait_time = (attempt + 1) * 5
                print(f"HF rate limited (attempt {attempt+1}/3), waiting {wait_time}s...")
                time.sleep(wait_time)
                continue
        ...
        # Step 2: Check if caption relates to water infrastructure issues
        water_keywords = [
        ...
        # Reject keywords — things that are clearly NOT water issues
        reject_keywords = [
