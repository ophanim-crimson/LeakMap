...
def classify_photo_with_ai(image_bytes: bytes) -> dict:
    """Use Gemini Vision REST API to classify whether the photo shows a water infrastructure issue."""
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
        return {"is_valid": None, "label": None}
    try:
        import base64
        b64_image = base64.b64encode(image_bytes).decode("utf-8")
        prompt = (
            "You are a photo classifier for a water leak reporting app. "
            "Analyze this image and determine if it shows a water infrastructure issue "
            "(water leak, pipe burst, overflow, broken tap, damaged valve, flooding, water supply problem, puddle from a leak, wet road from a burst pipe, etc). "
            "Respond in EXACTLY this format on two lines:\n"
            "VALID: true OR false\n"
            "LABEL: (a short 2-5 word description of what the image shows)\n"
            "Example valid responses:\n"
            "VALID: true\nLABEL: Leaking water pipe\n"
            "VALID: false\nLABEL: Selfie of a person"
        )
        payload = {
            "contents": [
                {
        ...
        resp = requests.post(url, headers={"X-goog-api-key": api_key, "Content-Type": "application/json"}, json=payload, timeout=30)
        ...
        for line in lines:
            ...
            if line_lower.startswith("valid:"):
                is_valid = "true" in line_lower
            elif line_lower.startswith("label:"):
                label = line.split(":", 1)[1].strip()
        return {"is_valid": is_valid, "label": label}
...
def get_ai_urgency(issue_type: str, description: str) -> str:
    ...
    if not api_key:
        return "Pending"
    try:
        prompt = f"Analyze this water infrastructure issue report.\nType: {issue_type}\nDescription: {description or 'No description'}\n\nDetermine the urgency of this issue. Respond with exactly one word from this list: Low, Medium, High, Critical."
        payload = {"contents": [{"parts": [{"text": prompt}]}]}
        url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent"
        resp = requests.post(url, headers={"X-goog-api-key": api_key, "Content-Type": "application/json"}, json=payload, timeout=30)
        resp.raise_for_status()
        result = resp.json()["candidates"][0]["content"]["parts"][0]["text"].strip().title()
        # Handle cases where AI adds punctuation like "Critical."
