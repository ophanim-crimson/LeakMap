...
def classify_photo_with_ai(image_bytes: bytes) -> dict:
    """Use Hugging Face image captioning to classify whether the photo shows a water infrastructure issue."""
    import time
    ...
    if hf_token:
        try:
            caption_url = "https://api-inference.huggingface.co/models/Salesforce/blip-image-captioning-large"
            headers = {"Authorization": f"Bearer {hf_token}"}
            
            resp = None
            for attempt in range(2):
                try:
                    resp = requests.post(caption_url, headers=headers, data=image_bytes, timeout=15)
            ...
            if resp and resp.status_code == 200:
                ...
                if caption:
                    water_keywords = [
                        "water", "leak", "pipe", "flood", "puddle", "burst", "tap", "faucet",
                        "drain", "sewer", "hydrant", "valve", "plumbing", "wet", "drip",
                        "overflow", "gutter", "manhole", "sprinkler", "hose", "stream",
                        "canal", "dam", "reservoir", "tank", "spill", "pool", "rain",
                        "broken pipe", "water damage", "sewage", "drainage", "waterfall",
                        "sink", "basin", "bathroom", "kitchen", "ground", "road", "street"
                    ]
                    
                    reject_keywords = [
                        "logo", "icon", "cartoon", "drawing", "illustration", "graphic",
                        "selfie", "portrait", "person smiling", "screenshot", "text",
                        "cat", "dog", "food", "car", "phone", "laptop", "screen",
                        "painting", "art", "anime", "game", "poster", "advertisement"
                    ]
                    
                    for keyword in reject_keywords:
                        if keyword in caption:
                            return {"is_valid": False, "label": caption.title()[:50]}
                    
                    is_water_related = any(keyword in caption for keyword in water_keywords)
                    if is_water_related:
                        return {"is_valid": True, "label": caption.title()[:50]}
                    else:
                        return {"is_valid": False, "label": f"Unrelated image ({caption.title()[:35]})"}
        except Exception as e:
            print(f"Hugging Face Vision Error: {e}")
    
    # Fallback: Basic Image Quality & Structure Check via PIL
    try:
        ...
        # Check if image is single solid color (spam/blank)
        extrema = img.getextrema()
        if extrema and all(e[0] == e[1] for e in (extrema if isinstance(extrema[0], tuple) else [extrema])):
            return {"is_valid": False, "label": "Blank/solid color image detected"}
            
        return {"is_valid": True, "label": "Photo verified"}
    except Exception:
        return {"is_valid": True, "label": "Photo attached"}
...
def upload_photo(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Classify photo with AI
        with open(filepath, "rb") as f:
            saved_bytes = f.read()
        classification = classify_photo_with_ai(saved_bytes)
        
        # If AI confirms the photo is NOT a water issue, reject it and delete the file
        if classification["is_valid"] is False:
            os.remove(filepath)
            label = classification.get("label", "unrelated content")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Photo rejected: This image appears to show \"{label}\" which is not a water infrastructure issue. Please upload a photo of the actual leak or water problem."
            )
        
        return schemas.PhotoBase(
            image_url=relative_url, 
            file_size=len(content),
            ai_is_valid=classification["is_valid"],
...
def create_report(payload: schemas.ReportCreate, db: Session = Depends(get_db)):
    ...
    # Evaluate urgency with Gemini API
...
def list_reports(
    ...
    # Filters
    print(f"DEBUG: status={status}, issue_type={issue_type}, q={q}")
    if status:
        if status == "Confirmed":
            query = query.join(models.Verification, models.Report.id == models.Verification.report_id)\
                         .filter(models.Report.status == "Active")\
                         .filter(models.Verification.verification_type == "Confirmed")
        else:
